...
static volatile int round_no = 0;
...
static void v_handler(void)
{
	if (!role_known) {
		...
		role_known = 1;
		if (role == 1)
			print_str("[V] uloga: PISAC\n");
		else
			print_str("[V] uloga: CITALAC\n");
		return;
	}
	...
	if (role == 1) {
		...
		confirmed = inl(ACK_PORT);

		print_str("[V] pisac runda ");
		print_num(round_no);
		print_str(": poslato ");
		print_num(n);
		print_str(" B, hipervizor potvrdio ");
		print_num((int)confirmed);
		print_str(" B\n");
